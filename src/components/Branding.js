...
const BrandingWrapper = styled.header`
  ${props => props.large && css`margin-bottom: ${shevy.baseSpacing(4)};`};
  ${props => props.large && css`color: ${themeValue('colors.foreground')}`};

  ${props =>
    !props.large &&
    css`
      border-top: 6px solid ${themeValue('colors.foreground')};
      background-color: ${props => props.backgroundColor && themeValue(props.backgroundColor)};
...
export const Branding = ({ backgroundColor, large, children, className }) => (
  <BrandingWrapper className={className} large={large} backgroundColor={backgroundColor}>
    <Inset>
